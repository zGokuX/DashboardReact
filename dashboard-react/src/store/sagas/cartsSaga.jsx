...
import { fetchCarts, fetchCartsBySingleUserId } from '@/services/requests'
import { fetchCartsRequest, fetchCartsSuccess, fetchCartsFailure, fetchSingleCartsRequest, fetchSingleCartsSuccess, fetchSingleCartsFailure } from '@/store/slices/cartsSlice'
...
function* handleFetchSingleCarts(action) {
  try {
    ...
    // In caso di errore inviamo un'azione di failure con il messaggio.
    yield put(fetchSingleCartsFailure(error.message || 'Failed to load carts'))
  }
}
...
export function* watchFetchSingleCart(){
  yield takeLatest(fetchSingleCartsRequest.type,handleFetchSingleCarts)
}